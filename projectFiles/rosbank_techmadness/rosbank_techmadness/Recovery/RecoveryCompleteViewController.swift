//
//  RecoveryCompleteViewController.swift
//  TechMadnessLogic
//
...
class RecoveryCompleteViewController: UIViewController {
            ...
            API.Registration.setSpeech(login: "test", index: 0, speech: self.wavData) { (success) in
                ...
                API.Registration.setWord(login: "test", index: 0, word: self.userword) { (success) in
                    ...
                    DispatchQueue.main.async {
                        let alert = UIAlertController(title: "Успешно", message: "Вы можете сменить пароль", preferredStyle: .alert)
                        
                        let acceptAction = UIAlertAction(title: "Далее", style: .default, handler: { [weak alert] (_) in
                            self.performSegue(withIdentifier: "ShowNewPassword", sender: self)
                        })
